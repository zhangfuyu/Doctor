...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(self.cancelBtnText, nil) style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
...
- (void)doneButtonPressed:(id)sender {
    ...
    if (_doneButton) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowserDidFinishModalPresentation:)]) {
            ...
            if ([self.cancelBtnText isEqualToString:@"删除    "]) {
                [_delegate photoBrowserDidFinishModalPresentation:self];
            }else if ([self.cancelBtnText isEqualToString:@"取消    "]) {
                [self dismissViewControllerAnimated:YES completion:nil];
            }
