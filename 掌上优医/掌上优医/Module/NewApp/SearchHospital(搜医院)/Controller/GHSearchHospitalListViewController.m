//
//  GHSearchHospitalListViewController.m
//  掌上优医
//
...
@interface GHSearchHospitalListViewController ()<UITableViewDelegate, UITableViewDataSource, GHProvinceCityAreaViewControllerDelegate, GHHospitalSortViewControllerDelegate, GHHospitalFilterViewControllerDelegate>
...
@implementation GHSearchHospitalListViewController
...
//    [SVProgressHUD showWithStatus:kDefaultTipsText];
    ...
    params[@"words"] = self.searchKey;
//    params[@"typeId"] = @(self.diseaseid);
    params[@"wordsType"] = @(4);
    params[@"page"] = @(self.hospitalCurrentPage);
...
//    
//    params[@"areaId"] = self.hospitalAreaId;
//    params[@"areaLevel"] = @(self.hospitalAreaLevel);
//    
//    //        1,"关键字相关度最高"
//    //        2,"离我最近"
//    //        3,"业务评分最高"
...
//    //        10,"综合排序"
//
//    if ([self.hospitalSortButton.currentTitle isEqualToString:@"等级最高"]) {
//        params[@"sortType"] = @(4);
...
//    }];
    ...
    [mapButton setTitleColor:kDefaultBlackTextColor forState:UIControlStateNormal];
    mapButton.titleLabel.font = H16;
    [mapButton setTitle:@"  附近医院" forState:UIControlStateNormal];
    [mapButton setImage:[UIImage imageNamed:@"hospital_list_map"] forState:UIControlStateNormal];
...
//    return 126;
    return  [GHNewHospitalTableViewCell getCellHeightFor:[self.hospitalArray objectOrNilAtIndex:indexPath.row]];
