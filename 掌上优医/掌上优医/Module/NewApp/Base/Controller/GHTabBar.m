...
//        addBtn.backgroundColor = kDefaultBlueColor;
        //设置默认背景图片
        [addBtn setBackgroundImage:[UIImage imageNamed:@"tabbar_center_add"] forState:UIControlStateNormal];
        //设置按下时背景图片
        [addBtn setBackgroundImage:[UIImage imageNamed:@"tabbar_center_add"] forState:UIControlStateHighlighted];
        //添加响应事件
...
-(void)addBtnDidClick
{
    if ([self.myTabBarDelegate respondsToSelector:@selector(addButtonClickAction)])
    {
        [self.myTabBarDelegate addButtonClickAction];
    }
}
...
-(void)layoutSubviews
{
    ...
    //设置“+”按钮的位置
    self.addButton.mj_x = SCREENWIDTH / 2.f - self.addButton.currentBackgroundImage.size.width / 2.f;
    self.addButton.mj_y = 49 * 0.5 - 1.5 * AddButtonMargin - self.addButton.currentBackgroundImage.size.height / 2.f;
//    self.addButton.centerY = self.height * 0.5 - 1.5 * AddButtonMargin;
    ...
    addLbl.text = @"写点评";
    addLbl.font = H10;
    addLbl.textColor = UIColorHex(0x999999);

    [addLbl sizeToFit];

    //设置“添加”label的位置
    addLbl.centerX = self.addButton.centerX;
    addLbl.centerY = CGRectGetMaxY(self.addButton.frame) + 0.5 * AddButtonMargin + 0.5 + 4.5;
