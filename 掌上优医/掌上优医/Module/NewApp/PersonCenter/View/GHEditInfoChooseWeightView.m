...
@interface GHEditInfoChooseWeightView ()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation GHEditInfoChooseWeightView
...
- (NSMutableArray *)dataArray {
    ...
    if (!_dataArray) {
        ...
        for (NSInteger index = 30; index <= 200; index++) {
...
- (void)clickConfirmAction {
    ...
    if ([self.delegate respondsToSelector:@selector(finishEditWeightWithText:)]) {
        
        NSInteger row = [self.pickerView selectedRowInComponent:0];
        
        
        [self.delegate finishEditWeightWithText:ISNIL([self.dataArray objectOrNilAtIndex:row])];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(UIView *)view{
    ...
    pickerLabel.text = [NSString stringWithFormat:@"%@kg", [self.dataArray objectOrNilAtIndex:row]];
