...
@implementation GHSegmentTitleView
...
*/
- (instancetype)initWithFrame:(CGRect)frame titles:(NSArray *)titlesArr delegate:(id<GHSegmentTitleViewDelegate>)delegate indicatorType:(FSIndicatorType)incatorType
{
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [GHSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [GHSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [GHSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
