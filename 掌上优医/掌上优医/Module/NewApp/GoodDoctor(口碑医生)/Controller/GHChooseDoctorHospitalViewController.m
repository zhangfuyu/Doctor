//
//  GHChooseDoctorHospitalViewController.m
//  掌上优医
//
...
@interface GHChooseDoctorHospitalViewController ()<UITableViewDelegate, UITableViewDataSource, GHProvinceCityAreaViewControllerDelegate, GHHospitalSortViewControllerDelegate, GHHospitalFilterViewControllerDelegate>
...
@implementation GHChooseDoctorHospitalViewController
...
- (void)requestData {
    ...
    params[@"hospitalLevel"] = levelString ? levelString : nil;
    ...
    [[GHNetworkTool shareInstance] requestWithMethod:GHRequestMethod_GET withUrl:kApiSearchHospital withParameter:params withLoadingType:GHLoadingType_ShowLoading withShouldHaveToken:YES withContentType:GHContentType_Formdata completionBlock:^(BOOL isSuccess, NSString * _Nonnull msg, id  _Nonnull response) {
        ...
        if (isSuccess) {
            ...
            for (NSDictionary *dicInfo in response[@"data"][@"hospitalList"]) {
                ...
                model.distance = [NSString stringWithFormat:@"%@",ISNIL(dicInfo[@"distance"])];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(choooseHospitalWithModel:)]) {
        [self.delegate choooseHospitalWithModel:model];
    }
