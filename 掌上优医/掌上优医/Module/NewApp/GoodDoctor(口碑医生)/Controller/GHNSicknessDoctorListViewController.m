//
//  GHNSicknessDoctorListViewController.m
//  掌上优医
//
...
@interface GHNSicknessDoctorListViewController ()<UITableViewDelegate, UITableViewDataSource, GHProvinceCityAreaViewControllerDelegate, GHDoctorSortViewControllerDelegate, GHDoctorFilterViewControllerDelegate>
...
@implementation GHNSicknessDoctorListViewController
...
//    [self requestData];
    ...
    [[GHNetworkTool shareInstance] requestWithMethod:GHRequestMethod_GET withUrl:kApiDisease withParameter:params withLoadingType:GHLoadingType_HideLoading withShouldHaveToken:false withContentType:GHContentType_Formdata completionBlock:^(BOOL isSuccess, NSString * _Nullable msg, id  _Nullable response) {
        ...
        if (isSuccess) {
            ...
            self.departmentName = ISNIL(response[@"firstDepartmentName"]);
...
- (void)setupUI {
    ...
    [self.view addSubview:self.doctorHeaderView];
    ...
    [tableView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.mas_equalTo(0);
        make.top.mas_equalTo(self.doctorHeaderView.mas_bottom);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        GHNSicknessDoctorTopTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GHNSicknessDoctorTopTableViewCell"];
        
        if (!cell) {
            cell = [[GHNSicknessDoctorTopTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"GHNSicknessDoctorTopTableViewCell"];
        }
        
        cell.titleLabel.text = ISNIL(self.sicknessName);
        cell.departmentLabel.text = [NSString stringWithFormat:@"就诊科室: %@", ISNIL(self.departmentName)];
