...
@interface _XSDXMLDictionaryParser : NSObject <NSXMLParserDelegate>
@end

@implementation _XSDXMLDictionaryParser {
...
+ (NSDictionary *)dictionaryWithXML:(id)xml {
    _XSDXMLDictionaryParser *parser = nil;
    if ([xml isKindOfClass:[NSString class]]) {
        parser = [[_XSDXMLDictionaryParser alloc] initWithString:xml];
    } else if ([xml isKindOfClass:[NSData class]]) {
        parser = [[_XSDXMLDictionaryParser alloc] initWithData:xml];
    }
