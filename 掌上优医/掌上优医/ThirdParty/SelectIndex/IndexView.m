...
- (void)initialiseAllTitles {
    ...
    self.itemMaxSize = [@"W" boundingRectWithSize:CGSizeMake(SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:self.titleFontSize]} context:nil].size;
    //标题视图布局
    for (int i=0; i<self.indexItems.count; i++) {
        ...
        if (title == UITableViewIndexSearch) {
            ...
            itemLabel.font = [UIFont systemFontOfSize:self.titleFontSize];
...
- (void)setSelectedIndex:(NSInteger)selectedIndex {
    ...
    if (newIndex >= 0 && newIndex < self.itemsViewArray.count) {
        ...
        newItemLabel.textColor = kDefaultBlueColor;
        //处理选中圆形
        //圆直径
...
- (void)reloadData {
    ...
    for (UILabel *label in self.itemsViewArray) {
