...
- (void)layoutSubviews
{
    ...
    if (self.backgroundImageView) {
        ...
        if (self.imageSize.width != self.bounds.size.width) {
            self.backgroundImageView.size = self.imageSize;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.isCornerRadius = self.isCornerRadius;
    if (self.isCornerRadius) {
        cell.layer.cornerRadius = HScaleHeight(5);
    }
    ...
    if (!cell.hasConfigured) {
        ...
        cell.signImage = self.signImage;
        cell.imageCount = self.imageURLStringsGroup.count;
    }
